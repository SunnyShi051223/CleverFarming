...
def run_crawler_pipeline():
    ...
    for category, url in TARGET_URLS.items():
        ...
        if scraped_data:
            added = store_to_mysql(scraped_data, category)
            total_added += added
            details.append({"category": category, "added": added, "found": len(scraped_data)})
            
    return {"success": True, "total_added": total_added, "details": details}
